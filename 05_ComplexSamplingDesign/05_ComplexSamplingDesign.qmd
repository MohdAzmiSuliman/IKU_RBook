...
#### Count the unweighted sample{#sec-csdunwcount}

0. To count the number of sample

#### Estimating Prevalence

0. To estimate prevalence using `survey::` package, the function of `svymean(_)`.
    -   if the outcome variable is factor type, both original factor type and converted numerical type can be used.
        ...
        -   if converted to numerical, binary `0` and `1` is a must, with `1` correspond to `Yes`.
        -   this is because when using `svymean(_)` to calculate prevalence, the function will calculate how many `1` since `0` does not have value.

1. Using function `svymean(_)` to calculate 

#### Total Sample and Estimated Population

Based on tutorial on [Count the unweighted sample](@sec-csdunwcount), can you try to calculate total sample?.  

The tutorial on estimated total population will be cover in @Bonus-II:-Population-Pyramid

## Bonus I: 

## Bonus II: Population Pyramid{}
